...
const Cart = () => {
     ...
     return (
          <div className='lg:px-14 sm:px-8 px-4 py-10'>
               ...
               <div>{cart && cart.length > 0 && cart.map((item, i) => <ItemContent key={i} {...item} />)}</div>
               ...
               <div className='border-t-[1.5px] border-slate-200 py-4 flex sm:flex-row sm:px-0 px-2 flex-col sm:justify-between gap-4'>
                    ...
                    <div className='flex text-sm gap-1 flex-col'>
                         ...
                         <Link className='w-full flex justify-end' to='/checkout'>
                              <button
                                   onClick={() => {}}
                                   className='font-semibold w-[300px] py-2 px-4s rounded-sm bg-custom-blue text-white flex items-center justify-center gap-2 hover:text-gray-300 transition duration-500'>
                                   <MdShoppingCart size={20} />
                                   Checkout
                              </button>
                         </Link>

                         <Link className='flex gap-2 items-center mt-2 text-slate-500' to='/productss'>
                              <MdArrowBack />
